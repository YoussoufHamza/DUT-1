...
public class EtudiantLicence implements Etudiant{
	double moyenne;
	int idEtudiant;
	...
	public boolean inscrireMatiere(int id_matiere, double note) {
		try {
			...
			if(rs.next()) {
				...
				rs = stat.executeQuery("SELECT nom_matiere from matiere where id_matiere ="+id_matiere+"");
				if(rs.next()) {
	...
	public void payerFraisScolaire(double montant) {
		try {
			...
			stat = con.createStatement();
			if(montant == FRAIS_SCOLARITE_LICENCE) {
	...
	public JScrollPane afficherBulletin() {
		...
		try {
			...
			stat = con.createStatement();
			stat2 = con.createStatement();
			rs  = stat.executeQuery("SELECT count(*) from inscription as ins, etudiant as et Where (ins.id_etudiant = "+this.idEtudiant+")");
			int nbrMat = 0;
			if(rs.next()) {
				nbrMat = rs.getInt(1);
			}
